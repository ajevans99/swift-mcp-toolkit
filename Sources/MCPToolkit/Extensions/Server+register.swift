...
extension Server {
  ...
  /// Registers the provided prompts with the MCP server.
  ///
  /// The server automatically responds to `prompts/list` with metadata generated by each template's
  /// ``MCPPromptTemplate/toPrompt()`` output and wires `prompts/get` to
  /// ``MCPPromptTemplate/render(arguments:)``.
  ///
  /// - Parameter prompts: The collection of prompt templates exposed to MCP clients.
  /// - SeeAlso: https://spec.modelcontextprotocol.io/specification/2024-11-05/server/prompts/
  public func register(prompts: [any MCPPromptTemplate]) async {
    self.withMethodHandler(ListPrompts.self) { _ in
      .init(prompts: prompts.map { $0.toPrompt() })
    }

    self.withMethodHandler(GetPrompt.self) { params async throws in
      guard let template = prompts.first(where: { $0.name == params.name }) else {
        throw MCPError.invalidParams("Unknown prompt: \(params.name)")
      }

      let messages = try await template.render(arguments: params.arguments)
      return .init(description: template.description, messages: messages)
    }
  }
}
